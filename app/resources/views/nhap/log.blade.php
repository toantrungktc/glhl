...
@section('diachi')
    <div class="col-sm-6">
        <ol class="breadcrumb float-sm-right">
			...
			<li class="breadcrumb-item"><a href="{{ url('/nhap/show/'.$id) }}">Chi tiết</a></li>
				...
				<div class="card-body">
					...
					<table id="log" class="table table-bordered table-striped">
						...
						<tbody>
							...
							<tr>
							    <th>{{ $index }}</th>
							    <th>{{ optional($log->causer)->name }}</th>
					...
					<table id="log_detail" class="table table-bordered table-striped">
						...
						<tbody>
							@foreach ($log_detail as $index => $detail)
							<tr>
							    <th>{{ $index }}</th>
							    <th>{{ optional($detail->causer)->name }}</th>
								<th>{{ $detail->description }}</th>
								<th>
									@foreach($detail->properties['attributes'] as $attribute2 => $new2) 
										@if ( $attribute2 == 'vattu_id')
										{{ $attribute2 }} : {{ DB::table('glhls')->where('id',$new2)->pluck('name')->first() }} <br>
										@else
										{{ $attribute2 }} : {{ $new2 }} <br>
										@endif
									@endforeach
								</th>
								@if ( isset($detail->properties['old']) )
								<td>
									@foreach($detail->properties['old'] as $attribute2 => $old2) 
										@if ( $attribute2 == 'vattu_id')
										{{ $attribute2 }} : {{ DB::table('glhls')->where('id',$old2)->pluck('name')->first() }} <br>
										@else
										{{ $attribute2 }} : {{ $old2 }} <br>
										@endif
									@endforeach
